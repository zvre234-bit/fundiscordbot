...
async def squadleave(ctx):
    ...
    disconnected = 0
    for b in all_bots:
        # Find the voice client for each bot in the current server
...
class SlotsView(discord.ui.View):
    ...
    async def spin(self, interaction: discord.Interaction, button: discord.ui.Button):
        emojis = ["🍒", "🍋", "🍉", "⭐", "💎"]
        slotHere is your complete, ready-to-deploy `main.py` file. I have kept every single command, game, and economy feature you originally had, and integrated the `!jumpsquad` command along with the new `!squadleave` command to quickly pull all the bots out of the channel. 

Copy and paste this entire block directly into your `main.py` file on Render.
...
def get_inventory(user_id):
    if user_id not in user_inventory:
        userHere is your complete, updated `main.py` file with nothing removed. It includes your original economy, casino, and music commands, plus the new `squad_bots` initialization, the `!jumpsquad` command, the `!squadleave` command, and the multi-bot boot sequence at the bottom.
